use crate::substrate::{ActionPolicy, Brain, BrainConfig, Stimulus};
...
fn switching_cost(brain: &mut Brain, policy: &mut ActionPolicy) -> f32 {
    // Drive food for a while, then threat; measure how many steps until action switches.
    ...
    for _ in 0..6 {
        ...
        // now switch to threat, measure time until avoid is chosen
...
fn print_report(r: &AssayReport) {
    // Intentionally plain text: low overhead, easy to diff later.
